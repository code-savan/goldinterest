  ...
  return (
    <div className="fixed inset-0 z-50 flex justify-end">
      ...
      >
        ...
        <div className="flex-1 overflow-auto px-6 py-6">
          {items.length === 0 ? (
            ...
            <div className="space-y-5">
              {items.map((item) => (
                <div key={`${item.product.id}-${item.size}-${item.color}`} className="flex gap-4 pb-5 border-b border-[#E8E6E1]">
        ...
        {items.length > 0 && (
          <div className="border-t border-[#E8E6E1] px-6 py-6 bg-white space-y-4">
            <div className="flex justify-between text-sm">
              <span className="text-[#6B6B6B]">Subtotal</span>
              <span className="font-medium">${subtotal.toFixed(2)}</span>
            </div>
            <div className="text-[11px] text-[#6B6B6B]">Free shipping — all orders. Duties included for EU.</div>
            <Link
              href="/checkout"
              onClick={() => setIsOpen(false)}
